		...
		<RadixDialog.Root open={open} onOpenChange={onOpenChange}>
			<RadixDialog.Portal>
				...
				<RadixDialog.Content
					...
					className={cn(
						"kb-dialog-content fixed left-1/2 top-1/2 z-50 w-[90vw] max-w-lg max-h-[85vh] flex flex-col rounded-lg border border-border-subtle bg-surface-1 shadow-2xl focus:outline-none",
						contentClassName,
					)}
				>
					{children}
...
export function DialogHeader({
	...
	return (
		<div className="flex items-center justify-between px-2 py-2 max-md:px-3 max-md:py-3 bg-surface-2 border-b border-border shrink-0 rounded-t-lg">
...
export function DialogFooter({ children }: { children: ReactNode }): React.ReactElement {
	return (
		<div className="flex justify-end gap-2 px-2 py-2 max-md:px-3 max-md:py-3 max-md:gap-3 bg-surface-2 border-t border-border shrink-0 rounded-b-lg">
			{children}
		</div>
	);
}
		...
		<RadixAlertDialog.Root open={open} onOpenChange={onOpenChange}>
			<RadixAlertDialog.Portal>
				<RadixAlertDialog.Overlay
					...
					style={{ animation: "kb-overlay-show 150ms ease" }}
				/>
				<RadixAlertDialog.Content className="kb-dialog-content fixed left-1/2 top-1/2 z-50 flex max-h-[85vh] w-[90vw] max-w-md flex-col rounded-lg border border-border-subtle bg-surface-1 shadow-2xl focus:outline-none">
					{children}
...
export function AlertDialogHeader({
	...
	return (
		<div
			className={cn(
				"px-2 py-2 max-md:px-3 max-md:py-3 bg-surface-2 border-b border-border shrink-0 rounded-t-lg",
				className,
			)}
		>
			{children}
...
export function AlertDialogFooter({
	...
	return (
		<div
			className={cn(
				"flex justify-end gap-2 px-2 py-2 max-md:px-3 max-md:py-3 max-md:gap-3 bg-surface-2 border-t border-border shrink-0 rounded-b-lg",
				className,
			)}
		>
			{children}
